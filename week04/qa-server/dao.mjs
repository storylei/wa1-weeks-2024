...
export const listQuestions = () => {
  ...
  return new Promise((resolve, reject) => {
    ...
    db.all(sql, (err, rows) => {
      if (err) reject(err);
      else if (rows === undefined) resolve({ error: 'No Questions!' });
      else {
        console.log(rows);
        let res = [];
        rows.forEach((row) => {
          res.push(new Question(row.id, row.text, row.email, row.date));
        });
        resolve(res);
      }
    });
...
export const getQuestion = (id) => {
  return new Promise((resolve, reject) => {
    ...
    db.get(sql, [id], (err, row) => {
      if (err) reject(err);
      else if (row === undefined) resolve({ error: 'Question not available, check the inserted id.' });
      else {
...
export const addQuestion = (question) => {
  return new Promise((resolve, reject) => {
    ...
    db.get(sql, [question.email], (err, row) => {
      if (err) reject(err);
      else if (row === undefined) resolve({ error: 'Author not available, check the inserted email.' });
      else {
